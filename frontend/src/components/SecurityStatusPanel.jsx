...
const SecurityStatusPanel = ({ areaStatus, intrusionActive, fireActive, activeZone, latestAlertTime, alertCount }) => {
  const isFireActive = Boolean(fireActive);
  const isIntrusionActive = Boolean(intrusionActive && !isFireActive);
  const isAlert = areaStatus === 'alert';
  ...
  return (
    <aside className="xl:sticky xl:top-24 z-40 w-full rounded-[28px] border border-slate-200 bg-white p-4 shadow-[0_16px_50px_rgba(15,23,42,0.08)]">
      <div className={`rounded-2xl border px-4 py-3 ${isAlert ? 'emergency-glow border-rose-200 bg-rose-50 text-rose-700 blink-badge' : 'border-emerald-200 bg-emerald-50 text-emerald-700'}`}>
        <p className="text-xs uppercase tracking-[0.28em] opacity-80">Live status</p>
        <p className="mt-2 text-lg font-semibold">{isAlert ? '🔴 AREA NOT SECURE' : '🟢 AREA SECURE'}</p>
      </div>
